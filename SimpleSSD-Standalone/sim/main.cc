...
int main(int argc, char *argv[]) {
  ...
  auto ssdConfig = initSimpleSSDEngine(&engine, pDebugLog, pDebugLog, argv[2]);// ssd simulation config file is taken as input.

  // Create Driver
  switch (simConfig.readUint(CONFIG_GLOBAL, GLOBAL_INTERFACE)) {
    case INTERFACE_NONE:
      pInterface = new SIL::None::Driver(engine, ssdConfig);
        printf("**********it is NONE interface******************\n");
      break;
    case INTERFACE_NVME:
      pInterface = new SIL::NVMe::Driver(engine, ssdConfig);
        printf("**********it is NVMe Interface******************\n");
      break;
    default:
  ...
  switch (simConfig.readUint(CONFIG_GLOBAL, GLOBAL_SIM_MODE)) { // what to execute trace or generator.. declaring the objects
