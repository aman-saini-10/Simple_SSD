...
RequestGenerator::RequestGenerator(Engine &e, BIL::BlockIOEntry &b,
  ...
  // Read config
  printf("Reading cofig file in the request generator \n");
...
void RequestGenerator::init(uint64_t bytesize, uint32_t bs) {
  printf("In the init of the request generator \n");
  if (offset > bytesize) {
...
void RequestGenerator::begin() {
  initTime = engine.getCurrentTick();
    printf("In the begin phase of the request generator\n");
...
void RequestGenerator::generateAddress(uint64_t &off, uint64_t &len) {// random address generation
  // This function generates address to access
  // based on I/O type, blocksize/align and offset/size
      printf("In the address generator of the request generator\n");
...
bool RequestGenerator::nextIOIsRead() {
  ...
  // io_count should not zero
      printf("In the next IO generator of the request generator\n");
...
void RequestGenerator::_submitIO(uint64_t) {
  BIL::BIO bio;
      printf("In the submit IO of the request generator with %lu and %lu\n",bio.offset, bio.length);
  ...
  std::cout << "Request ID: " << "\t"<<bio.id <<"Offset\t"<< bio.offset <<"Length\t"<< bio.length << "Type\t"<<(bio.type == BIL::BIO_READ ? "Read" : "Write")<<"\t"<<bio.submittedAt<<std::endl;
  //std::cout << "Offset: " << bio.offset << std::endl;
  //std::cout << "Length: " << bio.length << std::endl;
  //std::cout << "Type: " << (bio.type == BIL::BIO_READ ? "Read" : "Write") << std::endl;
  //std::cout<<"arrival Time"<<bio.submittedAt<<std::endl;
