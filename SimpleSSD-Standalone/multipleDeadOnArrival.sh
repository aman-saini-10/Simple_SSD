...
#input_folder="/home/waqar/Downloads/MSR_SimpleSSD/MSR1/ReadIntensiveTraces/" DeadPrediciton is not activated only the stats with different cmt sizes is to be checked
...
for cmt in "65568"; do
    output_folder="/home/waqar/MappingTableWork/mappingWrokStats/hcftl/DeadLRUVsLRU/predictorThrreshold/lru2_$cmt"
    ...
    for input_file in $input_files; do
        ...
        sed -i "91s/size_t cmtSize = [0-9]\+;/size_t cmtSize = $cmt;/" simplessd/icl/global_point.cc
        sed -i '88s/\(bool useDeadOnArrivalPredictor *= *\)\(true\)/\1false/' simplessd/icl/global_point.cc #flag is changed from true to false
        sed -i '90s/\(bool usePortionOfCacheAsShadow *= *\)\(true\)/\1false/' simplessd/icl/global_point.cc
