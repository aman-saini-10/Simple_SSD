...
void HIL::read(Request &req) {
  DMAFunction doRead = [this](uint64_t beginAt, void *context) {
    auto pReq = (Request *)context;// request from dma
    uint64_t tick = beginAt;
  //printf("Inside HIL Read Tick: %lu\n",tick);
    pReq->reqID = ++reqCount;
  //printf("Inside HIL Write Tick: %lu Lpn %lu Length %lu Offset %lu\n",tick,pReq->range.slpn,pReq->length,pReq->offset);
    debugprint(LOG_HIL,
               "READ  | REQ %7u | LCA %" PRIu64 " + %" PRIu64 " | BYTE %" PRIu64
               " + %" PRIu64,
               pReq->reqID, pReq->range.slpn, pReq->range.nlp, pReq->offset,
               pReq->length);// some of this information you will get from the none.cc

    ICL::Request reqInternal(*pReq);
    pICL->read(reqInternal, tick);// read request to the icl
    ...
    completionQueue.push(*pReq);// completion time updation.

    updateCompletion();//.. finish the request

    delete pReq;// delete the request
  };
...
void HIL::write(Request &req) {
 ...
 // printf("Inside HIL Write Tick: %lu Lpn %lu Length %lu Offset %lu\n",tick,pReq->range.slpn,pReq->length,pReq->offset);
    ...
    debugprint(LOG_HIL,
               ...
               pReq->reqID, pReq->range.slpn, pReq->range.nlp, pReq->offset,
               pReq->length);
//printf("In HIl Write with LPN %lu and StatingLPN %lu and Length %lu\n",pReq->offset,pReq->range.slpn,pReq->length);
...
void HIL::updateBusyTime(int idx, uint64_t begin, uint64_t end) {
  ...
  switch (idx) {
    case 0:
          //ReadBusyBucket[end-begin]++;
    break;
    case 1:
    // do it for the write case
         // WriteBusyBucket[end-begin]++;
    break;
    default:
