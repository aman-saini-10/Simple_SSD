...
//printf("RequestPalRead Block %u Page %u\n",req.blockIndex,req.pageIndex);
  printPPN(req, "READ");

  convertCPDPBP(req, list);
  
  //printf("Size of the read List %ld \n",list.size());
...
void PALOLD::write(Request &req, uint64_t &tick) {
  ...
  std::vector<::CPDPBP> list;
//printf("RequestPalWrite Block %u Page %u RegId %lu RegSubId %lu ",req.blockIndex,req.pageIndex,req.reqID,req.reqSubID);
 
  printPPN(req, "WRITE");
//printf("Tick %lu InWriteIo %u\t",tick,req.blockIndex);;
  convertCPDPBP(req, list);
  //printf("Size of the write List %ld \n",list.size());
  for (auto &iter : list) {
...
//eraseMapBucket[iter.Die]++;// this statement will give us the information that which 
/*infoSSD.BlockNum=iter.Block;
infoSSD.ChipNum=iter.Package;
infoSSD.DieNum=iter.Die;
infoSSD.PlaneNum=iter.Plane;
infoSSD.channelNum=iter.Channel;
infoSSD.ftlBlockIndex=req.blockIndex;
eraseMapBucket[eraser]=infoSSD;*/
    pal->submit(cmd, iter);
    stat.eraseCount++;
...
void PALOLD::convertCPDPBP(Request &req, std::vector<::CPDPBP> &list) {
  ...
  //if(eraserIO)  printf("FTL Id %u\t",req.blockIndex);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    switch (idx) {
      ...
      //  if(eraserIO)   printf("addr.Channel %u temp %lu\t",addr.Channel,tmp);
        break;
      case INDEX_PACKAGE:
     ...
     // if(eraserIO)     printf("addr.Package %u temp %lu\t",addr.Package,tmp);
        break;
      case INDEX_DIE:
     ...
     // if(eraserIO)   printf("addr.Die %u temp %lu\t",addr.Package,tmp);
        break;
      case INDEX_PLANE:
        if (!useMultiplaneOP) {
    ...
    // if(eraserIO)    printf("addr.Plane %u temp %lu\t",addr.Plane,tmp);
        break;
      default:
        break;
    }
...
//if(eraserIO) printf("addr.Block %u \n",addr.Block);
  // Index of ioFlag
  tmp = 0;
...
void PALOLD::printCPDPBP(::CPDPBP &addr, const char *prefix) {
  debugprint(LOG_PAL_OLD,
          ...
          /*printf(
             "Eraser %lu|%-5s | C %5u | W %5u | D %5u | P %5u | Block %5u | P %5u\n", eraser,prefix,
             addr.Channel, addr.Package, addr.Die, addr.Plane, addr.Block,
             addr.Page);*/
...
void PALOLD::write(::CPDPBP &addr, uint64_t &tick) {
  ::Command cmd(tick, 0, OPER_WRITE, param.superPageSize);
//printf("If this write is getting called or not at the address %lu\n",)
