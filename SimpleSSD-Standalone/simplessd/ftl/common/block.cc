...
Block::Block(uint32_t blockIdx, uint32_t count, uint32_t ioUnit)
  ...
  if (ioUnitInPage == 1) {
    ...
    pCachedBits=new Bitset(pageCount);
...
Block::Block(const Block &old)
  ...
  if (ioUnitInPage == 1) {
    ...
    *pCachedBits=*old.pCachedBits;
    cachedpage= old.cachedpage;
...
Block::Block(Block &&old) noexcept
    : idx(std::move(old.idx)),
      pageCount(std::move(old.pageCount)),
       cachedpage(std::move(old.cachedpage)),
  ...
  old.pCachedBits=nullptr;
  old.pLPNs = nullptr;
  old.ppLPNs = nullptr;
  old.blockAccessCount=0;
  old.lastAccessed = 0;
  old.firstAccessed=0;
  old.eraseCount = 0;
}
...
void Block::setBlockAccessCountZero(){
  blockAccessCount=0;
}
...
uint32_t Block::getCachedPageCount(){
  if(pCachedBits== nullptr)
  return 0;
  else
  return pCachedBits->count();
}
...
pCachedBits->set(pageIndex);
test_read++;
//printf("PErformed Read at the block %u cached in pages %u\n",this->getBlockIndex(),this->cachedpage);
  return read;
}
uint32_t Block::getBlockAccessCount()
{
...
bool Block::write(uint32_t pageIndex, uint64_t lpn, uint32_t idx,
                  uint64_t tick) {
  bool write = false;
//printf("PErformed Write at the block %u cached in pages %u\n",this->getBlockIndex(),this->cachedpage);
//blockAccessCount++;
  if (ioUnitInPage == 1 && idx == 0) {
  ...
  if (write) {
    ...
    if (ioUnitInPage == 1) {
     ...
     // cachedpage--;
      pLPNs[pageIndex] = lpn;// on each write the logical address of the page is also written to the block.
    }
    else {
...
bool Block:: isCached(uint32_t pageIndex) const {
    if (ioUnitInPage == 1) {
      return pCachedBits->test(pageIndex);
    } else {
      return pCachedBits->test(pageIndex);
    }
  }
bool Block::isPageValid(uint32_t pageIndex) const {
...
void Block::deCache(uint32_t pageIndex) {
...
void Block::test_decache(uint32_t pageIndex) {
  //if (ioUnitInPage == 1) {
...
bool Block::test_cached(uint32_t pageIndex) {
  if (pCachedBits == nullptr) {
